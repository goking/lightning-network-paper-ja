...
まとめると、Bitcoinはシーケンス番号（未承認トランザクションのmempool用の利用を想定）を持ってリリースされた。元々の振舞いは、mempool内のトランザクションの置換に使われていた。（より高いシーケンス番号を持ったトランザクションがあれば、それで置換）
...
この構造（Revocable Sequence Maturity Contracr: RSMC）は、とても詳しいコントラクト文言を持つ２つのパスを作る。
