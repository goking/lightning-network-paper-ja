...
AliceとBobの双方が(どちらのインプットがチャネルの総額を表しているのかを知るため)出資トランザクションへのインプットを交換し、さらに後々署名する際に使う一方の鍵を交換する。この鍵が出資トランザクションの2-of-2アウトプットに対して使われる。それは出資トランザクションを遣うには双方の署名が必要となるためである。言い換えると、出資トランザクションを遣うには、AliceとBobの双方の合意が必要ということになる。
