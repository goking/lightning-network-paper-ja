...
> 図2: 当事者双方はいつでも、どちらの確約トランザクションでもブロードキャストできるが、実際に出資トランザクションからの支出に成功するのはどちらか一方（の確約トランザクション）のみである。この方法は、当事者の一方が直近のトランザクションのブロードキャストを望まない場合には上手くいかない。 

新しい確定トランザクションが生成された結果、いずれの当事者も確定トランザクションをブロードキャストを許されるため、少ない出資金を受け取る当事者は、そのアウトプットが自身にとってより多くの価値をもつ確約トランザクションをブロードキャストする強いインセンティブを持つことになる。この結果として、チャネルはすぐさまクローズされ、出資金が盗難されてしまう。それ故、このモデルの下では、ペイメントチャネルの生成はままならない。
...
コントラクトの文言に実際の強制力を持たせるために、トランザクションの無効化が可能な確定トランザクションを構築する必要がある。この無効化は、トランザクションがブロックチェーンに取り込まれるときに関するデータを使うことによって、そして、検証パスを確かめるためのトランザクションの成熟度を使うことによって、達成できる。
...
シーケンス暗号が意図した目的は、ブロードキャストされていないトランザクションの置換に転用できそうである。
しかし、この高シーケンス番号置換の振舞いは、強制力はない。
Mempool内の古いトランザクションの置き換えは保証できなく、そして、ブロックが最新バージョンのトランザクションを含むことも保証できない
オフチェーンのトランザクションバージョンを強制する方法は、“time commitments”によるものである。

無効化可能なトランザクションは、一意の型の出力スクリプトを持つトランザクションの、一意の出力部分から作ることができる。この親の出力部は２つのRedeemパス（１つ目は、即座に取り戻しができ、２つ目は子のトランザクションのcomdirmationが最小値に到達したときのみに取り戻しができる）を持つ。

これは、子トランザクションが要求する最小confirmation数のシケーンス番号を作ることによって達成できる。重要な点は、この新しいシーケンス番号（を使った制御方法）の振舞いは、アウトプットと取戻し用(redeeming）トランザクションの間のブロックの数（the number of blocks）が、先に指定されたblock heightかどうか、検証されたoutputから送金することだけを許可する。

トランザクションはシーケンス番号の振舞い（シーケンス番号で定義されるブロック数制約2より生成）によって無効化させることができる。これは、親トランザクションがブロックチェーンに取り込まれて、定義されたブロック番号になった後に有効となる送金トランザクションとなる。

これは、デポジット保証になるようなアウトプットを持つ親トランザクション（無効化されていないことを証明する）という構造を作る。トランザクションがブロードキャストされた直後から、その送金が無効化されていないかどうか、ブロックチェーンに参加する誰もが検証するための期間がある。
もし、誰かが1000 comfirmationの遅延を持った無効化可能なトランザクションを許可した場合、出力トランザクションの構造は、次の2-of-2のmultisigを持つ：

```
2 <Alice1> <Bob1> 2 OP CHECKMULTISIG
```

しかし、送金しようとする子トランザクションは、nSequemce値=1000を持つ。このトランザクションは両カウンターパーティーが有効と検証する署名が必要となるので、両パーティーは署名の一部としてnSequence値=1000を含める。両パーティーは、彼らの裁量によって、もう一つ別のnSequence番号を持たないトランザクションを作ることを同意してもよい。
この構造（Revocable Sequence Maturity Contracr: RSMC）は、とても詳しいコントラクト条件を持つ２つのパスを作る。
コントラクト条件は以下：

1. すべての参加者は、このコントラクトを強制する出力を持つコントラクトに送金する
1. 両参加者は、一定期間（例では1,000回の承認回数）を待って、幾つかのコントラクトに基金を送ることを合意してもよい。
1. 片方、または双方は、将来のある日付まで、その支払いをブロードキャストしないことを選んでもよい。どちらか片方の参加者は、待ち状態にある間、いつでも基金を取り戻してもよい。
1. もし、だれもこの取り戻しトランザクションをブロードキャストしなければ、両方の参加者が新しい支払い条件への置き換えに合意していることを必要十分条件とし、それらは上の支払いを無効化する。新しい支払いトランザクションは、コントラクトが全世界に開示（つまりブロックチェーン上でブロードキャスト）されたあと、すぐに清算することができる。
1. コントラクトが開示され、そして、新しい支払い構成が清算されていないときにおいて、以前に無効化された支払い条件を、どちらかの参加者によって清算してもよい（そのため、新しい条件の強制については、どちらかの参加者の責任となる）
